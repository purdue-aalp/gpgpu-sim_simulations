...
extern __device__ INLINE2 void merge(const uint var1, const uint var2, const uint rep);
//extern __device__ INLINE2 void mergeCycle(const uint* const _list_, const uint _listSize_);
extern __device__ INLINE2 uint lockVarRep(uint& var);
extern __device__ INLINE2 uint lockPtr(uint ptr);
extern __device__ INLINE2 void decodeCurrPts(const uint x, uint* const _currVar_, uint* const _currVarSize_, uint* const _nextVar_, uint* const _nextVarSize_);
